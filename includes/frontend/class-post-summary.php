			...
			// Processing
				...
				// Hooks
					...
					// Filters
						...
						add_filter( 'the_excerpt', 'Boo_Valley_Library::remove_shortcodes' );
						...
						add_filter( 'wmhook_boo_valley_summary_continue_reading', __CLASS__ . '::continue_reading' );
	 ...
	 */
		...
		public static function excerpt( $excerpt ) {
			...
			// Requirements check
				...
				if ( post_password_required( $post_id ) ) {
					...
					if ( ! is_single( $post_id ) ) {
						...
						return esc_html__( 'This content is password protected.', 'boo-valley' ) . ' <a href="' . esc_url( get_permalink() ) . '">' . esc_html__( 'Enter the password to view it.', 'boo-valley' ) . '</a>';
			...
			// Processing
				...
				if (
						! is_single( $post_id )
						&& Boo_Valley_Library::has_more_tag()
					) {
				...
				// Adding "Continue reading" link
					...
					if (
							! is_single( $post_id )
							&& in_array( get_post_type( $post_id ), (array) apply_filters( 'wmhook_boo_valley_summary_continue_reading_post_type', array( 'post', 'page' ) ) )
						) {
						$excerpt .= apply_filters( 'wmhook_boo_valley_summary_continue_reading', '' );
					}
...
} // /Boo_Valley_Post_Summary

add_action( 'after_setup_theme', 'Boo_Valley_Post_Summary::init' );
