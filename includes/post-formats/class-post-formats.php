 ...
 * - `wmhook_boo_valley_pf_` - Boo_Valley_Post_Formats class specific hooks
 *
 ...
 * @example
 *
 *   $post_format_media = (string) Boo_Valley_Post_Formats::get();
 *
 ...
 * 100) Helpers
 */
final class Boo_Valley_Post_Formats {
				...
				// Hooks
					...
					// Filters
						...
						// Fix SSL URLs
							...
							add_filter( 'wmhook_boo_valley_pf_format_media_output', array( $this, 'fix_ssl_urls' ), 9999 );
	 ...
	 */
		...
		static public function get( $format = null ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_pf_get_pre', false, $format );
	 ...
	 */
		...
		static public function format_media( $post_id = null, $format = null ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_pf_format_media_pre', false, $post_id, $format );
			...
			// Helper variables
				...
				$meta_name = apply_filters( 'wmhook_boo_valley_pf_format_media_meta_name', 'post_format_media' );

				$supported_formats = apply_filters( 'wmhook_boo_valley_pf_format_media_formats', array( 'audio', 'gallery', 'image', 'video' ) );
				...
				// Premature output filtering
					...
					$output = apply_filters( 'wmhook_boo_valley_pf_format_media_output_pre', $output, $post_id, $format );

				// Force refresh (regenerate and resave) the post media meta field

					if (
							// When forced by hook
							apply_filters( 'wmhook_boo_valley_pf_format_media_force_refresh', false, $post_id, $format )
							// When no media saved
							|| empty( $output )
				...
				// Return if we have output
					...
					if ( $output ) {
						return apply_filters( 'wmhook_boo_valley_pf_format_media_output', $output, $post_id, $format );
					}
			...
			// Processing
				 ...
				 */
					...
					// Filter the output
						...
						$output = apply_filters( 'wmhook_boo_valley_pf_format_media_output', $output, $post_id, $format );
					...
					// Custom action hook
						...
						do_action( 'wmhook_boo_valley_pf_format_media', $output, $post_id, $format, $meta_name );
	 ...
	 */
		...
		static public function get_media_audio_video( $post_id ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_pf_get_media_audio_video_pre', false, $post_id );
		...
		static public function get_media_gallery( $post_id ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_pf_get_media_gallery_pre', false, $post_id );
			...
			// Processing
				...
				// Get image IDs array: from shortcode attribute or attached images
					...
					if ( false !== strpos( $output, 'ids="' ) ) {
						...
						$output = implode( ',', array_keys( (array) get_children( apply_filters( 'wmhook_boo_valley_pf_get_media_gallery_get_children_args', array(
								'post_parent'    => $post_id,
								'post_status'    => 'inherit',
		...
		static public function get_media_image( $post_id ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_pf_get_media_image_pre', false, $post_id );
			...
			// Processing
				...
				// Get the image ID if the image is uploaded, otherwise output the URL
					...
					if (
							class_exists( 'Boo_Valley_Library' )
							&& ( $image_id = Boo_Valley_Library::get_image_id_from_url( $output ) )
						) {
						$output = $image_id;
					}
...
} // /Boo_Valley_Post_Formats

add_action( 'after_setup_theme', array( 'Boo_Valley_Post_Formats', 'init' ) );
