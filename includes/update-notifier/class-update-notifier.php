 ...
 * 30) Get remote data
 */
final class Boo_Valley_Library_Update_Notifier {
	 ...
	 */
		...
		private function __construct() {
			...
			// Helper variables
				...
				self::$xml = self::get_remote_xml_data( BOO_VALLEY_UPDATE_NOTIFIER_CACHE_INTERVAL );
		...
		public static function menu() {
			...
			// Processing
				...
				if (
						...
						&& version_compare( self::$xml->latest, BOO_VALLEY_THEME_VERSION, '>' )
					) {

					add_theme_page(
						// page_title
						sprintf(
							esc_html_x( '%s Theme Updates', '%s stands for the theme name. Just copy it.', 'boo-valley' ),
							wp_get_theme( 'boo-valley' )->get( 'Name' )
						),
						// menu_title
						esc_html_x( 'Theme Updates', 'Admin menu title.', 'boo-valley' ) . ' <span class="update-plugins count-1"><span class="update-count">1</span></span>',
						// capability
						'switch_themes',
						// menu_slug
						'theme-update-notifier',
						// function
						'Boo_Valley_Library_Update_Notifier::page'
					);
			...
			// Processing
				...
				if (
					...
					$wp_admin_bar->add_menu( array(
							'id'    => 'update_notifier',
							'title' => sprintf( esc_html_x( '%s update', 'Admin bar notification link. %s: theme name.', 'boo-valley' ), wp_get_theme( 'boo-valley' )->get( 'Name' ) ) . ' <span id="ab-updates">1</span>',
	 ...
	 */
		...
		public static function page() {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_updater_page_pre', false );
			...
			// Processing
				...
				<div class="wrap update-notifier-wrap about-wrap">
					...
					<h1><strong><?php echo wp_get_theme( 'boo-valley' )->get( 'Name' ); ?></strong> Theme Updates</h1>
					...
					<div class="about-text">
						...
						echo 'You have version ' . BOO_VALLEY_THEME_VERSION . ' installed. <strong>Update to version ' . trim( self::$xml->latest ) . ' now.</strong>';
							...
							<ul>
								...
								<li>
									...
									<ol>
										...
										<li>Using your FTP client, rename the old theme folder (for example from <code>boo-valley</code> to <code>boo-valley-bak</code>).</li>
										...
										<li>After checking whether the theme works fine, delete the renamed old theme folder from the server (the <code>boo-valley-bak</code> folder in our case).</li>
									</ol>
	 ...
	 */
		...
		public static function get_remote_xml_data( $interval ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_updater_get_remote_xml_data_pre', false, $interval );
			...
			// Helper variables
				...
				$db_cache_field              = 'boo_valley_notifier_cache';
				$db_cache_field_last_updated = 'boo_valley_notifier_cache_last_updated';
				...
				// Check the cache
					...
					if (
						...
						// Cache doesn't exist, or is old, so refresh it
							...
							$response = wp_remote_get( esc_url( trailingslashit( wp_get_theme( 'boo-valley' )->get( 'AuthorURI' ) ) . 'updates/boo-valley/boo-valley-version.xml' ) );
...
} // /Boo_Valley_Library_Update_Notifier

add_action( 'init', 'Boo_Valley_Library_Update_Notifier::init' );
