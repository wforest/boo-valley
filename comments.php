...
<div class="comments-area-inner">
	...
	<h2 class="comments-title">
		...
		printf(
			esc_html( _nx( 'One comment on &ldquo;%2$s&rdquo;', '%1$s comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'Comments list title.', 'boo-valley' ) ),
	...
	if ( have_comments() ) {
		...
		// If comments are closed and there are comments, let's leave a little note, shall we?
			...
			if (
				...
				<p class="comments-closed no-comments"><?php esc_html_e( 'Comments are closed.', 'boo-valley' ); ?></p>
		...
		// Are there comments to navigate through?
			...
			if (
				...
				<nav id="comment-nav-below" class="pagination comment-pagination" role="navigation" aria-labelledby="comment-nav-below-label" data-current="<?php echo esc_attr( $current ); ?>" data-total="<?php echo esc_attr( $total ); ?>">
					...
					<h2 class="screen-reader-text" id="comment-nav-below-label"><?php esc_html_e( 'Comments Navigation', 'boo-valley' ); ?></h2>

					<?php

					paginate_comments_links( (array) apply_filters( 'wmhook_boo_valley_pagination_args', array(
