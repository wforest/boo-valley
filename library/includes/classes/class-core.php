 ...
 * 100) Helpers
 */
final class Boo_Valley_Library {
	 ...
	 */
		...
		public static function theme_upgrade() {
			...
			// Helper variables
				...
				$current_theme_version = get_transient( 'boo-valley_version' );
				$new_theme_version     = wp_get_theme( 'boo-valley' )->get( 'Version' );
			...
			// Processing
				...
				if (
					...
					do_action( 'wmhook_boo_valley_library_theme_upgrade', $current_theme_version, $new_theme_version );

					set_transient( 'boo-valley_version', $new_theme_version );
	 ...
	 */
		...
		public static function add_table_of_contents( $content = '' ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_pre', false, $content );
			...
			// Helper variables
				...
				$title_text = apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_title_text', sprintf( esc_html_x( '"%s" table of contents', '%s: post title.', 'boo-valley' ), the_title_attribute( 'echo=0' ) ) );
				$title      = apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_title', '<h2 class="screen-reader-text">' . $title_text . '</h2>' );

				$args = apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_args', array(
			...
			// Processing
				...
				// Get post parts titles
					...
					foreach ( $args['post_content'] as $part ) {
						...
						// Get the title for post part
							...
							if ( $args['disable_first'] && 1 === $i ) {
								...
								if ( ! isset( $matches[2] ) || ! $matches[2] ) {
									$part_title = sprintf( esc_html__( 'Page %s', 'boo-valley' ), number_format_i18n( $i ) );
								} else {
						...
						// Post part item output
							...
							$args['links'][$i] = (string) apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_part', '<li' . $class . '>' . _wp_link_page( $i ) . $part_title . '</a></li>', $i, $part_title, $class, $args );
				...
				// Add table of contents into the post/page content
					...
					$links = apply_filters( 'wmhook_boo_valley_library_add_table_of_contents_links', array(
		...
		public static function get_the_paginated_suffix( $tag = '', $singular_only = false ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_get_the_paginated_suffix_pre', false, $tag, $singular_only );
			...
			// Processing
				...
				if ( 1 < $paged ) {
					$output = ' ' . $tag[0] . sprintf( esc_html_x( '(page %s)', 'Paginated content title suffix, %s: page number.', 'boo-valley' ), number_format_i18n( $paged ) ) . $tag[1];
				}
		...
		public static function has_more_tag( $post = null ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_has_more_tag_pre', false, $post );
	 ...
	 */
		...
		public static function remove_shortcodes( $content ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_remove_shortcodes_pre', false, $content );
		...
		public static function link_skip_to( $id = 'content', $text = '', $class = '' ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_link_skip_to_pre', false, $id, $text, $class );
			...
			// Helper variables
				...
				if ( empty( $text ) ) {
					$text = esc_html__( 'Skip to main content', 'boo-valley' );
				}
		 ...
		 *
		 * Hook onto `wmhook_boo_valley_library_contextual_help_texts_array` to add help texts.
		 *
		 * @example
		 *
		 *   $texts_array = array(
		...
		public static function contextual_help( $contextual_help, $screen_id, $screen ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_contextual_help_pre', false, $contextual_help, $screen_id, $screen );
			...
			// Helper variables
				...
				$texts_array = array_filter( (array) apply_filters( 'wmhook_boo_valley_library_contextual_help_texts_array', array() ) );
		...
		public static function get_image_id_from_url( $url ) {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_get_image_id_from_url_pre', false, $url );
			...
			// Helper variables
				...
				$cache = array_filter( (array) get_transient( 'boo_valley_image_ids' ) );
			...
			// Processing
				...
				// Cache the new record
					...
					set_transient( 'boo_valley_image_ids', array_filter( (array) $cache ) );
			...
			public static function image_ids_transient_flusher() {
				...
				// Processing
					...
					delete_transient( 'boo_valley_image_ids' );
		...
		public static function is_categorized_blog() {
			...
			// Pre
				...
				$pre = apply_filters( 'wmhook_boo_valley_library_is_categorized_blog_pre', false );
			...
			// Processing
				...
				if ( false === ( $all_cats = get_transient( 'boo_valley_all_categories' ) ) ) {
					...
					set_transient( 'boo_valley_all_categories', $all_cats );
			...
			public static function all_categories_transient_flusher() {
				...
				// Processing
					...
					// Like, beat it. Dig?
						...
						delete_transient( 'boo_valley_all_categories' );
...
add_action( 'after_setup_theme', 'Boo_Valley_Library::init', -50 );
...
if ( ! function_exists( 'get_theme_file_uri' ) ) {
	function get_theme_file_uri( $file = '' ) {
		...
		// Output
			...
			return Boo_Valley_Library::get_theme_file_uri( $file );
