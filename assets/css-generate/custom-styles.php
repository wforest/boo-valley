 ...
 */
function boo_valley_custom_styles( $visual_editor = false ) {

	// Pre

		$pre = apply_filters( 'wmhook_boo_valley_custom_styles_pre', false, $visual_editor );
	...
	// Helper variables
		...
		$helper = apply_filters( 'wmhook_boo_valley_custom_styles_helper', array(
				'layout_width_site'    => get_theme_mod( 'layout_width_site', 1640 ),
				'layout_width_content' => get_theme_mod( 'layout_width_content', 1180 ),
	...
	// Processing
		 ...
		 *
		 * You can hook onto `wmhook_boo_valley_custom_styles_use_custom_array` and disable the theme
		 * default array setup. Then just hook onto `wmhook_boo_valley_custom_styles_array` to create
		 ...
		 */
		if ( ! apply_filters( 'wmhook_boo_valley_custom_styles_use_custom_array', false ) ) {
		...
		} // /wmhook_boo_valley_custom_styles_use_custom_array

			// Filter the $custom_styles array

				$custom_styles = apply_filters( 'wmhook_boo_valley_custom_styles_array', $custom_styles, $visual_editor, $helper );
								...
								// When CSS selector and styles set up
									...
									foreach ( $selector['styles'] as $property => $style ) {
										if ( trim( $style ) ) {
											...
											// This is for multiple overrode properties
		 ...
		 */
		if ( is_callable( 'Boo_Valley_Library_Customize_Styles::custom_styles' ) ) {

			$custom_styles_custom_types = apply_filters( 'wmhook_boo_valley_custom_styles_custom_types', array(
					'frontend' => array(
							'addon',
						),
					'editor'   => array(
			...
			$output .= "\r\n\r\n" . Boo_Valley_Library_Customize_Styles::custom_styles( ob_get_clean() );

		}

		// Filter the output

			$output = (string) apply_filters( 'wmhook_boo_valley_custom_styles_output', $output, $visual_editor, $helper );
	...
	// Output
		...
		return apply_filters( 'wmhook_boo_valley_esc_css', $output );
